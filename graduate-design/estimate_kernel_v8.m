function K = estimate_kernel_v8(B,K_grd,opt)
...
for s = 2:length(ker_scale)
    ...
    for T = 1:20
        ...
        %
        M1 = 2 * R + opt.sparse.* W + opt.smooth.*(Dkx' * Wx * Dkx + Dky' * Wy * Dky);
        [u, sig, v] = svd(M1);
        ...
        while ~converge
            ...
            tminv = diag(sig) + mu * ones(length(sig),1);
            sinv = 1./tminv;
            Sinv = diag(sinv);
            
            % update the kernel k
            lambda = ((C * v) * Sinv * (u' * C'))\((C * v) * Sinv * (u' * (mu * x - y)) - e);
            k = v * Sinv * (u' * (mu * x - y)) - v * Sinv * (u' * C' * lambda);
