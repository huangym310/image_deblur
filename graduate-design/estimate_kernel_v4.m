function K = estimate_kernel_v4(B, opt)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% blind kernel estimation from multiple image.
%%
%% + GST for L_p sparse regularization comparing to v2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%    b =  2*(para.mu1 * e' * C + para.mu2 * u' + para.mu3 * (gx'* Dkx + gy' *Dky) + para.mu4 * w');
    b = 2*(para.mu1 * e' * C + para.mu2 * u' + para.mu4 * w');
    k = (1/2) * inv_A * b';
    
    u = GST(k, para.lam1/(2 * para.mu2), p, J);
    
    w = max(k,0);
    
%     kx = Dkx * k;
%     ky = Dky * k;
%     gx = GST(kx, para.lam2 /(2*para.mu3), p, J);
%     gy = GST(ky, para.lam2 /(2*para.mu3), p, J);
