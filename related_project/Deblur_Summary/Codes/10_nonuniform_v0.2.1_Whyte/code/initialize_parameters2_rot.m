function [x1,x2] = initialize_parameters2_rot(obs,blur,im,true_blur,true_im,initoptions,spatial_mask,options)
	% Author: Rob Fergus
	...
	if non_uniform && ~isempty(true_blur)
	...
	elseif strcmp(mode_blur,'zbar')
		if non_uniform
			...
			error('Cant''t initialize uniform blur to zbar');
		end
	...
	elseif strcmp(mode_im,'variational')
		...
		if non_uniform
			...
			% Mask out region unaffected by edge effects
			Dpf0 = mask_observation_border_mex(ones([I,J/2]),[I,J/2],Kblurry,Kblurry,-[ttx(:),tty(:),ttz(:)]',ones(K,L),1,1,-[0;0;0]) > 0.5;
